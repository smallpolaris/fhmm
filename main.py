...
def do(o):
    ...
    # print("name : %s staring, n:%d"% (name, o),  flush=True, file=open("fhmm" + str(o) + ".txt", "a"))
    N = 200
    NT = 4
    NX = 3
    NV = NX + 1
    ...
    # folder = "../v_3/"
    # folder = "../v3_gamma/"
    x = torch.load(folder + "t_x.pt")
    y = torch.load(folder + "t_y.pt")
    t_state = torch.load(folder + "t_state.pt")
    t_state = t_state.long()
    h = torch.load(folder + "t_h.pt")
    ...
    for iter in range(Iter):
        ...
        beta, accept_beta = data.update_vc(y, x, state, beta, sigma, thres, bs_value, eta, ptau, c_beta,
                                           accept_beta, tau_beta)  # 0.06
        thres, accept_thres = data.update_thres(y, x, state, beta, sigma, thres, bs_value, eta, c_thres,
                                                accept_thres)  # 0.04
        ptau = data.update_ptau(beta)
        ...
        all_P[r, iter] = P
        all_Z[r, iter] = Z
        L = data.update_L(P, Z, y, mean, state, sigma)
        all_L[r, iter] = L
